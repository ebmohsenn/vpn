// ...existing code...
// Settings page section for Check-Host nodes
...
function vpnsm_settings_page() {
	...
	<div class="wrap">
		...
		<form method="post" action="options.php" id="vpnsm-settings-form">
			<?php settings_fields('vpnsm_settings'); do_settings_sections('vpnsm_settings'); ?>
			<table class="form-table">
				<tr valign="top">
					<th scope="row"><?php esc_html_e('Check-Host Nodes', 'vpnserver'); ?></th>
					<td id="vpnsm-checkhost-nodes-container">
						<div id="vpnsm-nodes-list">
						<?php foreach ($nodes as $host => $label): ?>
							<label style="display:block;margin-bottom:2px;">
								<input type="checkbox" name="vpnsm_checkhost_nodes[]" value="<?php echo esc_attr($host); ?>" <?php checked(in_array($host, $selected)); ?> />
								<?php echo esc_html($label); ?> <code><?php echo esc_html($host); ?></code>
							</label>
						<?php endforeach; ?>
						</div>
						<button type="button" class="button" id="vpnsm-load-nodes-btn"><?php esc_html_e('Load Nodes', 'vpnserver'); ?></button>
						<p class="description"><?php esc_html_e('Select one or more official Check-Host nodes. Only nodes from the official list can be used. No manual addition.', 'vpnserver'); ?></p>
					</td>
				</tr>
			</table>
			<?php submit_button(); ?>
	...
	document.addEventListener('DOMContentLoaded', function() {
		document.getElementById('vpnsm-load-nodes-btn').addEventListener('click', function() {
			var btn = this;
			btn.disabled = true;
			btn.textContent = 'Loading...';
			fetch(ajaxurl + '?action=vpnsm_refresh_nodes&_wpnonce=<?php echo wp_create_nonce('vpnsm_refresh_nodes'); ?>')
				.then(r => r.json())
				.then(data => {
					var list = document.getElementById('vpnsm-nodes-list');
					list.innerHTML = '';
					if (data && data.nodes) {
						Object.entries(data.nodes).forEach(([host, label]) => {
							var checked = (data.selected && data.selected.includes(host)) ? 'checked' : '';

...
$type_class = $type === 'premium' ? 'type-premium' : 'type-standard';
				...
				<div class="vpn-card vpnpm-card" data-search="<?php echo esc_attr($search_haystack); ?>">
					...
					<p>Ping (Check-Host): <span class="vpnpm-ping-ch"<?php echo $chErr ? ' title="' . esc_attr($chErr) . '"' : ''; ?>><?php echo ($server->checkhost_ping_avg !== null ? esc_html((int)$server->checkhost_ping_avg) . ' ms' : esc_html__('N/A', 'vpnserver')); ?></span><?php if ($chErr): ?> <small class="description" style="color:#d63638;">(<?php echo esc_html__('error', 'vpnserver'); ?>)</small><?php endif; ?></p>
					<?php endif; ?>
